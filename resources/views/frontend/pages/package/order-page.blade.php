...
@section('page-title')
    {{__('Order For')}} {{' : '.$order_details->lang_front->title}}
@endsection
@section('content')
                ...
                <div class="right-content-area">
                    <div class="single-price-plan-01 active">
                        <div class="price-header">
                            ...
                            <h4 class="title">{{$order_details->lang_front->title}}</h4>
                        </div>
                        <div class="price-body">
                            <ul>
                                @foreach(explode("\n",$order_details->lang_front->features) as $item)
                                    <li><i class="fa fa-check success"></i> {{$item}}</li>
                                @endforeach
                            </ul>
                        </div>
                        <div class="price-wrap">
                            {!! custom_amount_with_currency_symbol($order_details->price) !!} <span class="sign">{{ $order_details->lang_front->type }}</span>
                        </div>
                        <div class="price-footer">
                            <div class="btn-wrapper">
                                <a href="{{ ($order_details->btn_url)?? route('frontend.plan.order',['id' => $order_details->id]) }}" class="boxed-btn">{{$order_details->lang_front->btn_text}}</a>
                            </div>
                        </div>
                    </div>
                </div>
