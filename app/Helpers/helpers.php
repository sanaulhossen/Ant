...
function render_payment_gateway_for_form($cash_on_delivery = false)
{
    ...
    $all_gateway = [
        'paypal', 'manual_payment', 'mollie', 'paytm', 'stripe', 'razorpay', 'flutterwave', 'paystack','SSLCommerce'
    ];
    $output .= '<ul>';
    if ($cash_on_delivery) {
...
function render_menu_by_id($id)
{
    ...
    foreach ($menu_content as $menu_item) {
        ...
        if ($menu_item->ptype == 'static') {
        ...
        } elseif ($menu_item->ptype == 'service' || $menu_item->ptype == 'event' || $menu_item->ptype == 'blog' || $menu_item->ptype == 'product' || $menu_item->ptype == 'appointment') {

            if ($menu_item->ptype == 'service') {
                $menu_title = '';
                $menu_slug = '';

                $page_details = \App\Services::with('lang_front')->find($menu_item->pid);
                if (!empty($page_details)){
                    $menu_title = $page_details->lang_front->title;
                    $menu_slug = route('frontend.services.single', [$page_details->lang_front->slug,$page_details->id]);
                    $li_class .= (request()->is(route('frontend.services.single', [$page_details->lang_front->slug,$page_details->id]))) ? ' current-menu-item ' : '';
                }
                ...
                $menu_title = '';
                $menu_slug = '';
                $page_details = \App\Products::with('lang_front')->find($menu_item->pid);
                if (!empty($page_details)){
                    $menu_title = $page_details->lang_front->title;
                    $menu_slug = route('frontend.products.single', [$page_details->lang_front->slug,$page_details->id]);
                    $li_class .= (request()->is(route('frontend.products.single', [$page_details->lang_front->slug,$page_details->id]))) ? ' current-menu-item ' : '';
                }
            ...
            $mega_menu_items = \App\Services::with('lang_front')->find($mega_menu_ids)->groupBy('categories_id');
            ...
            $menu_slug = url('/') . '/' . $menu_raw_path;
            $li_class .= (request()->is($menu_raw_path)) ? ' current-menu-item ' : '';
            $mega_menu_items = \App\Products::with('lang_front')->find($mega_menu_ids)->groupBy('category_id');
        ...
        if (!empty($menu_slug) && !empty($menu_title)){//start condition
            ...
            if (!empty($mega_menu_enable)) {
                ...
                if($menu_item->ptype == 'product_mega_menu'){
                    foreach ($mega_menu_items as $cat => $posts) {
                        ...
                        foreach ($posts as $post) {
                            ...
                            $output .= '<a href="'.$mega_menu_item_slug.'"><h4 class="title">'.purify_html($post->lang_front->title).'</h4></a>';
...
function render_submenu_children($menu_children)
{
    ...
    foreach ($menu_children as $menu_item) {
        ...
        if ($menu_item->ptype == 'static') {
            ...
            $menu_slug = str_replace('@url', url('/'), $menu_item->purl);
            $li_class .= (request()->is($menu_slug)) ? ' current-menu-item ' : '';
        } elseif ($menu_item->ptype == 'service' || $menu_item->ptype == 'blog' || $menu_item->ptype == 'appointment' ||  $menu_item->ptype == 'product') {
            $menu_title = '';
            $menu_slug = '';
            if ($menu_item->ptype == 'service') {
                ...
                if(!empty($page_details)){
                    $menu_title = $page_details->lang_front->title;
                    $menu_slug = route('frontend.services.single', [$page_details->lang_front->slug, $page_details->id]);
                    $li_class .= (request()->is(route('frontend.services.single', [$page_details->slug, $page_details->id]))) ? ' current-menu-item ' : '';
                }
...
function render_draggable_menu_by_id($id)
{
    ...
    foreach ($menu_data as $menu) :
        ...
        if ($menu->ptype == 'static') {
        ...
        } elseif ($menu->ptype == 'service' || $menu->ptype == 'blog' || $menu->ptype == 'product' || $menu->ptype == 'appointment' ) {
            $menu_attr .= ' data-pid="' . $menu->pid . '"';
            if ($menu->ptype == 'service') {
                $menu_details = \App\Services::with('lang_front')->find($menu->pid);
                $menu_title = !empty($menu_details) ? $menu_details->lang_front->title : '' ;
            ...
            elseif ($menu->ptype == 'product') {
                $menu_details = \App\Products::with('lang_front')->find($menu->pid);
                $menu_title = !empty($menu_details) ?  $menu_details->lang_front->title : '' ;
            ...
            $mega_menu_enable = 'mega_menu';
            $mega_menu_items = \App\Services::with('lang_front')->where(['status' => 'publish'])->get();
        } elseif ($menu->ptype == 'product_mega_menu') {
            $menu_title = get_static_option('product_page_' . $default_lang . '_name') . __(' Mega Menu');
            $mega_menu_enable = 'mega_menu';
            $mega_menu_items = \App\Products::with('lang_front')->where(['status' => 'publish'])->get();
    ...
    foreach ($children as $item) {
        ...
        if ($item->ptype == 'static') {
            ...
            if ($item->ptype == 'service') {
                $menu_details = \App\Services::with('lang_front')->find($item->pid);
                $menu_title = !empty($menu_details) ? $menu_details->lang_front->title : '';
            } elseif ($item->ptype == 'blog') {
                $menu_details = \App\Blog::with('lang_front')->find($item->pid);
                $menu_title = !empty($menu_details) ? $menu_details->lang_front->title : '';
            } elseif ($item->ptype == 'product') {
                $menu_details = \App\Products::with('lang_front')->find($item->pid);
                $menu_title = !empty($menu_details) ? $menu_details->lang_front->title : '';
