...
class ServiceController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            foreach ($all_languages as $lang){
                $lang_slug = $lang->slug;
                ServicesLang::where(['service_id' => $request->id,'lang' => $lang_slug])->update([
                'title' => $request->title[$lang_slug],
                'description' => $request->description[$lang_slug],
                'excerpt' => $request->excerpt[$lang_slug],
                'meta_description'=> $request->meta_description[$lang_slug],
    ...
    public function category_update(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            ServiceCategory::find($request->id)->update(['status' => $request->status,'icon'=>$request->icon])->id;
            foreach ($all_languages as $lang){
                $lang_slug = $lang->slug;
                ServiceCategoryLang::where(['category_id' => $request->id,'lang' => $lang->slug])->update([
                    'lang' => $lang_slug,
                    'name' =>  $request->name[$lang_slug]
                ]);
            }
