...
class FooterRecentBlogPostWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li class="single-recent-post-item">
                    <div class="thumb">' . render_image_markup_by_attachment_id($post->blog_front->image, '', 'thumb') . '</div>
                    <div class="content">
                        <h4 class="title"><a href="' . route('frontend.blog.single',['slug' => $post->blog_front->slug,'id' => $post->id ]) . '">' . purify_html($post->blog_front->title) . '</a></h4>
                        <span class="time"> <i class="far fa-calendar-alt '.$icon_class.'"></i>' . date_format($post->blog_front->created_at, 'd M Y') . '</span>
                    </div>
                </li>';
        }
        $output .= '</ul>';
    ...
    public function widget_title()
    {
        ...
        return __('Footer Recent Blog Post');
    }
}